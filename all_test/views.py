...
from final_control.models import *
...
def test(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'GET':
            ...
            if Lesson.objects.filter(name=lesson_check).exists():
                user = request.user
                context['user'] = user
                if lesson_check:
                    ...
                    if user and lesson:
                        ...
                        if start:
                            context['start'] = True
                        context['old_results'] = UserTestsResult.objects.filter(user=user,lesson=lesson)
        ...
        if request.method == 'POST':
            ...
            user = request.user
            lesson = Lesson.objects.filter(name=lesson_check).first()
            result = UserTestsResult.objects.create(user=user,lesson=lesson)
            context['old_results'] = UserTestsResult.objects.filter(user=user,lesson=lesson)
            context['user'] = user
            context['lesson'] = lesson
